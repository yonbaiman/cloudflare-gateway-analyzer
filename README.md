...
**Break the "Free Tier" limit. Export detailed DNS logs and visualize your network security.** **無料プランの壁を越える。詳細なDNSログを抽出し、ネットワークセキュリティを可視化します。**
...
Instead of using real private data, this project provides a **Live Demo** generated by a specialized script. This report perfectly reflects the actual layout of the GitHub Actions Step Summary.  
(プライバシー保護のため、実データの代わりに専用スクリプトで自動生成された**ライブデモ**を公開しています。実際のサマリー画面と同じレイアウトを完全に再現しています。)
